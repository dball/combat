var Combat = {
  ...
  init: function(json, id, viewport_id, controls_id) {
    ...
    this.effects.init(json.map.effects);
    this.map.viewport.reset();
    this.actions.bind($('#' + id), $('#' + controls_id));
  },

  draw: function() {
    this.map.draw(this.pictures, this.walls, this.effects, this.figures, this.actions);
  },

  things: function() {
    return this.figures.all.concat(this.pictures.all, this.effects.all, this.walls.all);
  }
}
